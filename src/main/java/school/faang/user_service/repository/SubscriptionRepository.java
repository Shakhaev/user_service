...
public interface SubscriptionRepository extends CrudRepository<User, Long> {
    ...
    @Query(nativeQuery = true, value = "insert into subscription (follower_id, followee_id) values (:followerId, "
            + ":followeeId)")
    @Modifying
    ...
    @Query(nativeQuery = true, value = "select exists(select 1 from subscription where follower_id = "
            + ":followerId and followee_id = :followeeId)")
