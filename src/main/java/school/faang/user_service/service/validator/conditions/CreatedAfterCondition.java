...
@Component
public class CreatedAfterCondition implements RecommendationRequestCondition {
    @Override
    public boolean matches(RecommendationRequest request, RequestFilterDto filter) {
        return filter.getCreatedAfter() == null || request.getCreatedAt().isBefore(filter.getCreatedAfter());
    }
}
